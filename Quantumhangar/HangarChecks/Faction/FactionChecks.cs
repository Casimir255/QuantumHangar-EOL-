...
namespace QuantumHangar.HangarChecks
{
    ...
    public class FactionChecks
    {
        ...
        private FactionHangar FactionsHangar { get; set; }
        ...
        private bool PerformMainChecks(bool isSaving)
        {
            ...
            FactionsHangar = new FactionHangar(SteamId, _chat);
            if (FactionsHangar.CheckPlayerTimeStamp()) return true;
        ...
        public async void SaveGrid()
        {
            ...
            if (!FactionsHangar.ExtensiveLimitChecker(gridData))
                return;
            ...
            FactionsHangar.SelectedFactionFile.FormatGridName(gridData);

       

            var val = await FactionsHangar.SaveGridsToFile(result, gridData.GridName);
            if (val)
            {
                FactionsHangar.SaveGridStamp(gridData);
        ...
        public void DetailedInfo(string input)
        {
            ...
            if (!FactionsHangar.ParseInput(input, out var id))
            {
                _chat.Respond($"Grid {input} could not be found!");
                return;
            }

            FactionsHangar.DetailedReport(id);
        }
        ...
        public async void LoadGrid(string input, bool loadNearPlayer)
        {
            ...
            if (!FactionsHangar.TryGetGridStamp(id, out var stamp))
                return;
            ...
            if (!FactionsHangar.LoadGrid(stamp, out var grids, _identityId))
            {
            ...
            if (!FactionsHangar.CheckLimits(stamp, grids, _identityId))
                return;
            ...
            if (spawner.Start(spawnPos, keepOriginalPosition))
            {
                ...
                FactionsHangar.RemoveGridStamp(id);
            }
            else
            {
        ...
        public void SellGrid(int id, long price, string description)
        {
            ...
            if (!FactionsHangar.SellSelectedGrid(stamp, price, description))
                return;
        ...
        public void RemoveGrid(string input)
        {
            ...
            if (FactionsHangar.RemoveGridStamp(id))
