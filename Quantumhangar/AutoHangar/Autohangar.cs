            ...
            try
            {
                ...
                foreach (var group in MyCubeGridGroups.Static.Physical.Groups.ToList())
                {
                    ...
                    foreach (var gridList in ByPlayer)
                    {
                        ...
                        if (hangarStatic && playersOfflineDays[gridList.Key] >= staticDays || saveAll)
                        {
                            exportedGrids.AddRange(gridList.Value.Where(x => x.GridSizeEnum == MyCubeSize.Large && x.IsStatic).ToList());
                        }
                        if (hangarLarge && playersOfflineDays[gridList.Key] >= largeDays || saveAll)
                        {
                            exportedGrids.AddRange(gridList.Value.Where(x => x.GridSizeEnum == MyCubeSize.Large && !x.IsStatic).ToList());
                        }
                        if (hangarSmall && playersOfflineDays[gridList.Key] >= smallDays || saveAll)
                        {
